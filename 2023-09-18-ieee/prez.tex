...
\begin{frame}{What about error correction?}
	...
	\begin{columns}[t]
		\begin{column}{0.5\textwidth}
			...
			% \only<3>{
			% 	\begin{tikzpicture}[domain=0:4, samples=100, scale=0.75]%, overlay]
			% 		\draw[very thin,color=gray] (-0.1,-0.1) grid (3.9,3.9);

			% 		\draw[->] (-0.2,0) -- (4.2,0) node[below] {$\langle O\rangle$};
			% 		\draw[->] (0,-0.2) -- (0,4.2) node[left] {count};

			% 		\draw[color=black]  plot (\x,{3*exp(-5*(\x - 2)^2)})          node[right] {ideal};

			% 		\draw[color=red]    plot (\x,{2.6*exp(-4*(\x - 1)^2)})          node[above] {noisy};
			% 		\fill[fill=red!20, opacity=0.5]   plot (\x,{2.6*exp(-4*(\x - 1)^2)});

			% 		\draw[color=blue]   plot (\x,{exp(-1*(\x - 2)^2)})             node[right] {mitigated};
			% 		\fill[fill=blue!20, opacity=0.5]   plot (\x,{exp(-1*(\x - 2)^2)});
			% 	\end{tikzpicture}
			% }
			\uncover<4->{
